...
try {
  ...
  for (const f of [
    ...
    "20260917000040_support_automation.sql",
  ])
  ...
  const signatures = {
    ...
    support_automation_policy: ["p_case"],
    claim_support_auto_ack: [
      "p_draft",
      "p_expected",
      "p_case_version",
      "p_kind",
  ...
  try {
    ...
    const { automationRun, automationRule } =
      await import("../server-handlers/support/automation.ts");
    process.env.GEMINI_API_KEY = "fixture";
    process.env.GEMINI_SUPPORT_MODEL = "fixture";
    await db.query(
      "insert into support_messages(case_id,gmail_message_id,gmail_thread_id,direction,sender_email,recipient_email,subject,body_text,sent_at)values($1,'inbound4','thread1','INBOUND','customer@example.com','support@example.com','Question','Login help',clock_timestamp())",
      [id],
    );
    draft = await makeDraft();
    const ruleInput = {
    ...
    let rule = await rpc("save_support_automation", [
      null,
      null,
      ruleInput,
      admin,
    ]);
    eq(rule.auto_send_enabled, true);
    eq((await rpc("support_automation_policy", [id])).can_auto_ack, true);
    assert.throws(() => automationRule({ ...ruleInput, subcategory: null }));
    checks++;
    assert.throws(() =>
      automationRule({ ...ruleInput, minimum_confidence: 0.89 }),
    );
    checks++;
    assert.throws(() =>
      automationRule({
        ...ruleInput,
        category: "SYSTEM",
        subcategory: "SYSTEM_OUTAGE",
      }),
    );
    checks++;
    eq(
      (
        await rpc("save_support_automation", [
          rule.id,
    ...
    await db.query("update support_cases set ai_confidence=0.89 where id=$1", [
      id,
    ]);
    eq((await rpc("support_automation_policy", [id])).can_auto_ack, false);
    await db.query(
      "update support_cases set ai_confidence=0.99,urgency='HIGH' where id=$1",
      [id],
    );
    eq((await rpc("support_automation_policy", [id])).can_auto_ack, false);
    await db.query("update support_cases set urgency='NORMAL' where id=$1", [
      id,
    ]);
    const verification = randomUUID();
    await db.query(
      "insert into support_knowledge(id,knowledge_code,title,category,subcategory,problem,resolution,active,requires_port_verification,human_review_required)values($1,'VERIFY','Verification','ACCOUNT','LOGIN','Issue','Verify with staff',true,true,true)",
      [verification],
    );
    eq((await rpc("support_automation_policy", [id])).can_auto_ack, false);
    await db.query("update support_knowledge set active=false where id=$1", [
      verification,
    ]);
    const oldVersion = rule.updated_at;
    rule = await rpc("save_support_automation", [
      rule.id,
      rule.updated_at,
      ruleInput,
      admin,
    ...
    eq(
      (
        await rpc("claim_support_auto_ack", [
          draft.id,
          draft.updated_at,
          await caseVersion(),
          "SEND",
    ...
    checks++;
    eq(
      (await rpc("save_support_automation", [null, null, ruleInput, admin]))
        .error,
      "DUPLICATE_SCOPE",
    );
    eq((await run(automationRun, { case_id: id }, false)).status, 401);
    const before = writes;
    delete process.env.SUPPORT_AUTO_SEND_ENABLED;
    let automatic = await run(automationRun, { case_id: id });
    eq(automatic.status, 200);
    eq(writes, before);
    process.env.SUPPORT_AUTO_SEND_ENABLED = "true";
    let ack = (await rpc("get_support_reply", [id, null])).draft;
    ack = (
      await rpc("edit_support_reply", [
        ack.id,
        ack.updated_at,
        "Staff added operational text.",
        admin,
      ])
    ).draft;
    automatic = await run(automationRun, { case_id: id });
    eq(automatic.status, 409);
    eq(writes, before);
    const fixed =
      "Thank you for contacting CargoMove. We have received your enquiry. Further guidance requires review by our support team.";
    ack = (
      await rpc("edit_support_reply", [ack.id, ack.updated_at, fixed, admin])
    ).draft;
    automatic = await run(automationRun, { case_id: id });
    eq(automatic.status, 200);
    eq(writes, before + 1);
    const autoOp = automatic.result.operation;
    eq(autoOp.approval_mode, "AUTOMATIC_ACK");
    eq(autoOp.automation_rule_id, rule.id);
    const autoInteraction = (
      await db.query(
        "select approved_by,approved_at,final_reply from ai_interactions where id=$1",
        [ack.interaction_id],
      )
    ).rows[0];
    eq(autoInteraction.approved_by, null);
    eq(autoInteraction.approved_at, null);
    eq(autoInteraction.final_reply, fixed);
    eq((await run(automationRun, { case_id: id })).status, 409);
    eq(writes, before + 1);
    eq(
      (
        await db.query(
          "select count(*)::int n from support_automation_rule_events",
        )
      ).rows[0].n,
      2,
    );
    for (const role of ["anon", "authenticated"]) {
      eq(
        (
          await db.query(
            "select has_function_privilege($1,'support_automation_policy(uuid)','EXECUTE')ok",
            [role],
          )
        ).rows[0].ok,
        false,
      ...
      eq(
        (
          await db.query(
            "select has_function_privilege($1,'save_support_automation(uuid,timestamptz,jsonb,uuid)','EXECUTE')ok",
            [role],
          )
        ).rows[0].ok,
        false,
  ...
  console.log(
    `${checks} isolated automation and delivery database/API checks passed.`,
  );
} finally {
  await db.close();
}
